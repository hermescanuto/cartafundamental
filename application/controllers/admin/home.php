...
class Home extends CI_Controller {
	...
	private function pag_conf()
	{
		...
		$this->data['css']= array(
				...
				array('css_url' => 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.9.2/themes/base/jquery-ui.css'),
	...
	public function index()
	{
		...
		$rec['id_1']=$data['id1'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id1']);		
		$recordset['legenda1']  = $x['titulo'];

		$rec['id_2']=$data['id2'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id2']);		
		$recordset['legenda2']  = $x['titulo'];

		$rec['id_3']=$data['id3'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id3']);		
		$recordset['legenda3']  = $x['titulo'];
		...
		$rec['id_4']=$data['id1'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id1']);		
		$recordset['legenda4']  = $x['titulo'];
     
		$rec['id_5']=$data['id2'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id2']);		
		$recordset['legenda5']  = $x['titulo'];

		$rec['id_6']=$data['id3'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id3']);		
		$recordset['legenda6']  = $x['titulo'];
		...
		$rec['id_7']=$data['id1'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id1']);		
		$recordset['legenda7']  = $x['titulo'];
     

		$rec['id_8']=$data['id2'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id2']);		
		$recordset['legenda8']  = $x['titulo'];

		$rec['id_9']=$data['id3'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id3']);		
		$recordset['legenda9']  = $x['titulo'];
		...
		$rec['id_10']=$data['id1'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id1']);		
		$recordset['legenda10']  = $x['titulo'];
		

		$rec['id_11']=$data['id2'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id2']);		
		$recordset['legenda11']  = $x['titulo'];

		$rec['id_12']=$data['id3'];
		$x= $this -> Model_util -> ByIDtoTemplate('vw_conteudo', $data['id3']);		
		$recordset['legenda12']  = $x['titulo'];
